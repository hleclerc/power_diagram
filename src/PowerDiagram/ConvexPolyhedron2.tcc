...
template<class Pc,class CI>
void ConvexPolyhedron2<Pc,CI>::for_each_boundary_measure( FunctionEnum::Unit, const std::function<void(TF,CI)> &f ) const {
    if ( nb_points == 0 ) {
        if ( sphere_radius >= 0 )
...
template<class Pc,class CI>
void ConvexPolyhedron2<Pc,CI>::add_centroid_contrib( Pt &ctd, TF &mea, FunctionEnum::Unit ) const {
    using std::pow;
...
template<class Pc,class CI>
void ConvexPolyhedron2<Pc,CI>::add_centroid_contrib( Pt &ctd, TF &mea, FunctionEnum::Gaussian ) const {
    TODO;
    ...
    //    _r_centroid_integration( ctd.x, ctd.y, coeffs_centroid );
    ...
    //    TF lea = _r_polynomials_integration( coeffs );
    //    ctd += lea * _sphere_center;
    //    mea += lea;
}

template<class Pc,class CI>
void ConvexPolyhedron2<Pc,CI>::add_centroid_contrib( Pt &ctd, TF &mea, FunctionEnum::R2 ) const {
    TODO;
...
typename ConvexPolyhedron2<Pc,CI>::Pt ConvexPolyhedron2<Pc,CI>::centroid( const FU &f ) const {
    TF mea = 0;
    Pt ctd = { 0, 0 };
    add_centroid_contrib( ctd, mea, f );
...
template<class Pc,class CI> template<class FU>
typename Pc::TF ConvexPolyhedron2<Pc,CI>::measure( const FU &f ) const {
...
template<class Pc,class CI>
typename Pc::TF ConvexPolyhedron2<Pc,CI>::integration( FunctionEnum::Gaussian ) const {
    TODO;
    return 0;
    ...
    //    return _r_polynomials_integration( coeffs );
}
...
template<class Pc,class CI>
typename Pc::TF ConvexPolyhedron2<Pc,CI>::integration( FunctionEnum::Unit ) const {
...
template<class Pc,class CI>
typename Pc::TF ConvexPolyhedron2<Pc,CI>::integration( FunctionEnum::R2 ) const {
    TODO;
    return 0;
    //    using std::pow;
...
void ConvexPolyhedron2<Pc,CI>::_r_centroid_integration( TF &r_x, TF &r_y, const Coeffs &coeffs ) const {
    TODO;
    //    using std::pow;
    ...
    //                R_0 = R_2+R_0; c = R_0; R_2 = (-1.0)*R_4; R_5 = P1.x;
    ...
    //        TF c = 0;
    //        const auto &poly_coeffs = coeffs[ cut_index_r( pow( _sphere_radius, 2 ) ) ].second;
    //        for( std::size_t d = 0; d < poly_coeffs.size(); ++d )
    //            c += poly_coeffs[ d ] * pow( _sphere_radius, 2 * d + 1 );
    //        c *= 0.5;
    ...
    //    for( std::size_t i1 = 0, i0 = _cuts.size() - 1; i1 < _cuts.size(); i0 = i1++ ) {
    //        if ( _cuts[ i0 ].seg_type == SegType::line )
    //            int_seg( r_x, r_y, _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );
    //        else
    //            int_arc( r_x, r_y, _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );
    //    }
}
...
typename Pc::TF ConvexPolyhedron2<Pc,CI>::_r_polynomials_integration( const Coeffs &coeffs ) const {
    ...
    //        TF res = 0, r2 = pow( _sphere_radius, 2 );
    ...
    //    for( std::size_t i1 = 0, i0 = _cuts.size() - 1; i1 < _cuts.size(); i0 = i1++ )
    //        if ( _cuts[ i0 ].seg_type == SegType::arc )
    //            res += int_arc( _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );
    //        else
    //            res += int_seg( _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );
    //    return res;
}

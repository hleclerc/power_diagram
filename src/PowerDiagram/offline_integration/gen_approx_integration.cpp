...
#include "lib/UnivariatePiecewisePolynomialApproximator.h"
...
void gen_int_rad( std::ostream &os, const UnivariatePiecewisePolynomial<TF> &coeffs, bool centroid ) {
    using std::abs;

    auto only_one_coeff = [&]( int &coeff_index, TF &coeff_val, int num_cut ) {
        coeff_index = -1;
        for( std::size_t i = 0; i < coeffs.cuts[ num_cut ].coeffs.size(); ++i ) {
            if ( abs( coeffs.cuts[ num_cut ].coeffs[ i ] ) > 1e-80 ) {
                if ( coeff_index >= 0 )
                    return false;
    ...
    if ( coeffs.cuts.size() == 1 ) {
        ...
        if ( only_one_coeff( coeff_index, coeff_val, 0 ) ) {
            os << "auto arc_val = [&]( PT P0, PT P1 ) {\n";
            os << "    using std::atan2;\n";
            os << "    using std::pow;\n";
            os << "    TF a0 = atan2( P0.y, P0.x );\n";
            os << "    TF a1 = atan2( P1.y, P1.x );\n";
            os << "    if ( a1 < a0 )\n";
            os << "        a1 += 2 * M_PI;\n";
            os << "    return ( a1 - a0 ) * " << std::setprecision( 16 ) << coeff_val << " * pow( _sphere_radius, " << 2 * ( coeff_index + 1 ) << " );\n";
            os << "};\n";
            os << "\n";
            ...
            switch ( coeff_index ) {
            case 0:
                os << "    return " << std::setprecision( 16 ) << coeff_val * ( - 0.5 ) << " * ( ( P0.x + P1.x ) * ( P0.y - P1.y ) - ( P0.x - P1.x ) * ( P0.y + P1.y ) );\n";
                break;
            case 1:
                os << "    TF result; \n";
            ...
            std::cerr << "TODO: several coeffs in an unique polynomial.\n";
        }

        os << "TF lea = 0; \n";
        os << "if ( _cuts.empty() ) {\n";
        os << "    if ( _sphere_radius > 0 )\n";
        os << "        lea = 2 * M_PI * " << std::setprecision( 16 ) << coeff_val << " * pow( _sphere_radius, " << 2 * ( coeff_index + 1 ) << " );\n";
        os << "} else {\n";
        os << "    for( size_t i1 = 0, i0 = _cuts.size() - 1; i1 < _cuts.size(); i0 = i1++ ) {\n";
        os << "        if ( _cuts[ i0 ].seg_type == SegType::arc )\n";
        os << "            lea += arc_val( _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );\n";
        os << "        else\n";
        os << "            lea += seg_val( _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );\n";
        os << "    }\n";
        os << "}\n";
        ...
        os << "    static const std::vector<std::pair<TF,std::array<TF," << coeffs.cuts[ 0 ].coeffs.size() << ">>> coeffs = {\n";
        ...
        os << "    \n";
        if ( centroid )
            os << "    TF lea = _r_polynomials_integration( coeffs );\n";
        else
...
template<class Func>
void div_inv_integration( const Func &func, double precision, const std::vector<TF> &r, int degp, bool centroid ) {
    // evaluation of the function
    std::vector<TF> y( r.size() );
    for( size_t i = 0; i < r.size(); ++i )
        y[ i ] = func( r[ i ] );


    // get a piecewise polynomial approximation of func( r )
    UnivariatePiecewisePolynomialApproximator<TF> ap_func;
    ap_func.precision = precision / 10;
    ap_func.degp = 7; // we're using high precision number, so that it's legitimate to use high degree polynomials
    auto pa_f = ap_func.run( r, y );

    // get points of i( s ) = integrate( r * f_ap( r ), r = 0 .. s^0.5 ) / s.
    TF acc = 0;
    std::vector<TF> integ_ss;
    std::vector<TF> integ_ys;
    std::vector<TF> integ_ps; // ponderation
    for( std::size_t num_point = 0, num_cut = 0; num_point < r.size(); ++num_point ) {
        while ( r[ num_point ] >= pa_f.cuts[ num_cut ].position ) {
            TF beg = num_cut ? pa_f.cuts[ num_cut - 1 ].position : 0;
            TF end = pa_f.cuts[ num_cut ].position;
            for( std::size_t i = 0; i < pa_f.cuts[ num_cut ].coeffs.size(); ++i )
                acc += pa_f.cuts[ num_cut ].coeffs[ i ] / ( i + 2 ) * ( pow( end, i + 2 ) - pow( beg, i + 2 ) );
            ++num_cut;
        }
        integ_ps.push_back( r[ num_point ] );

        TF s = pow( r[ num_point ], 2 );
        integ_ss.push_back( s );

        TF v = acc;
        TF beg = num_cut ? pa_f.cuts[ num_cut - 1 ].position : 0;
        for( std::size_t i = 0; i < pa_f.cuts[ num_cut ].coeffs.size(); ++i )
            v += pa_f.cuts[ num_cut ].coeffs[ i ] / ( i + 2 ) * ( pow( r[ num_point ], i + 2 ) - pow( beg, i + 2 ) );
        integ_ys.push_back( v / s );
    }

    //    auto to_double = []( const std::vector<TF> &inp ) {
    //        std::vector<double> res( inp.size() );
    //        for( std::size_t i = 0; i < inp.size(); ++i )
    //            res[ i ] = double( inp[ i ] );
    //        return res;
    //    };
    //    matplotlibcpp::plot( to_double( integ_ss ), to_double( integ_ys ) );
    //    matplotlibcpp::show();

    // approximation of i( s )
    UnivariatePiecewisePolynomialApproximator<TF> ap_is;
    ap_is.precision = precision;
    ap_is.degp = degp;

    auto pa_i = ap_is.run( integ_ss, integ_ys, integ_ps );

    // code gen
    gen_int_rad( std::cout, pa_i, centroid );
}

void gen_int_centroid( std::ostream &os, const UnivariatePiecewisePolynomial<TF> &coeffs ) {
    using std::abs;

    os << "\n";
    os << "// centroid part\n";
    os << "ctd += lea * _sphere_center;\n";
    os << "mea += lea;\n";
    os << "\n";
    os << "if ( _cuts.empty() )\n";
    os << "    return;\n";
    ...
    auto only_one_coeff = [&]( int &coeff_index, TF &coeff_val, int num_cut ) {
        coeff_index = -1;
        for( std::size_t i = 0; i < coeffs.cuts[ num_cut ].coeffs.size(); ++i ) {
            if ( abs( coeffs.cuts[ num_cut ].coeffs[ i ] ) > 1e-50 ) {
                if ( coeff_index >= 0 )
                    return false;
                coeff_val = coeffs.cuts[ num_cut ].coeffs[ i ];
                coeff_index = i;
    ...
    if ( coeffs.cuts.size() == 1 ) {
        ...
        int coeff_index = 0;
        if ( only_one_coeff( coeff_index, coeff_val, 0 ) ) {
            os << "auto arc_val_centroid = [&]( TF &r_x, TF &r_y, PT P0, PT P1 ) {\n";
            os << "    using std::atan2;\n";
            os << "    using std::pow;\n";
            os << "    using std::sin;\n";
            os << "    using std::cos;\n";
            ...
            os << "    if ( a1 < a0 )\n";
            os << "        a1 += 2 * M_PI;\n";
            os << "    TF c = ( " << std::setprecision( 16 ) << 0.5 * coeff_val << " ) * pow( _sphere_radius, " << 1 + 2 * coeff_index << " );\n";
            os << "    r_x += c * ( sin( a1 ) - sin( a0 ) );\n";
            os << "    r_y += c * ( cos( a0 ) - cos( a1 ) );\n";
            os << "};\n";
            os << "\n";

            os << "auto seg_val_centroid = []( TF &r_x, TF &r_y, PT P0, PT P1 ) {\n";
            // generated using `metil src/PowerDiagram/offline_integration/lib/gen_centroid.met`
            switch ( coeff_index ) {
            case 0:
                os << "    r_x += ( " << std::setprecision( 16 ) << - coeff_val << " ) * ( P0.y - P1.y );;\n";
                os << "    r_y += ( " << std::setprecision( 16 ) <<   coeff_val << " ) * ( P0.x - P1.x ) };\n";
                break;
            case 1:
                os << "    TF c = ( " << std::setprecision( 16 ) << coeff_val / 8
                   << " ) * ( pow( P0.x + P1.x, 2 ) + pow( P0.y + P1.y, 2 ) ) + ( " << std::setprecision( 16 ) << coeff_val / 24
                   << " ) * ( pow( P1.x - P0.x, 2 ) + pow( P1.y - P0.y, 2 ) );\n";
                os << "    r_x += ( P1.y - P0.y ) * c;\n";
                os << "    r_y += ( P0.x - P1.x ) * c;\n";
                break;
            case 2:
                // -1.0/80.0*(P0.y-P1.y)*" << coeff_val << "*pow(pow(P1.x-P0.x,2)+pow(P1.y-P0.y,2),2)-1.0/16.0*(P0.y-P1.y)*" << coeff_val << "*pow(pow(P1.x+P0.x,2)+pow(P1.y+P0.y,2),2)-1.0/6.0*(P0.y-P1.y)*" << coeff_val << "*(1.0/2.0*pow((P1.y-P0.y)*(P1.y+P0.y)+(P1.x-P0.x)*(P1.x+P0.x),2)+1.0/4.0*(pow(P1.x-P0.x,2)+pow(P1.y-P0.y,2))*(pow(P1.x+P0.x,2)+pow(P1.y+P0.y,2)))
                os << "    TF R_0 = P0.x; TF R_1 = (-1.0)*R_0; TF R_2 = P1.x; TF R_3 = (-1.0)*R_2;\n";
                os << "    R_3 = R_0+R_3; R_0 = R_2+R_0; TF R_4 = pow(R_0,2); R_1 = R_2+R_1;\n";
                os << "    R_0 = R_1*R_0; R_1 = pow(R_1,2); R_2 = " << coeff_val << "; TF R_5 = P1.y;\n";
                os << "    TF R_6 = (-1.0)*R_5; TF R_7 = P0.y; TF R_8 = R_5+R_7; TF R_9 = pow(R_8,2);\n";
                os << "    R_9 = R_4+R_9; R_4 = pow(R_9,2); R_4 = R_2*R_4; TF R_10 = R_3*R_4;\n";
                os << "    R_10 = (1.0/32.0)*R_10; TF R_11 = (-1.0)*R_7; R_11 = R_5+R_11; R_8 = R_11*R_8;\n";
                os << "    R_0 = R_8+R_0; R_0 = pow(R_0,2); R_0 = 0.5*R_0; R_11 = pow(R_11,2);\n";
                os << "    R_11 = R_1+R_11; R_9 = R_11*R_9; R_9 = 0.25*R_9; R_9 = R_0+R_9;\n";
                os << "    R_9 = R_2*R_9; R_0 = R_3*R_9; R_0 = (1.0/12.0)*R_0; R_0 = R_10+R_0;\n";
                os << "    R_11 = pow(R_11,2); R_11 = R_2*R_11; R_3 = R_3*R_11; R_3 = (1.0/160.0)*R_3;\n";
                os << "    R_0 = R_3+R_0; r_y += R_0; R_6 = R_7+R_6; R_9 = R_6*R_9;\n";
                os << "    R_9 = (-1.0/12.0)*R_9; R_4 = R_6*R_4; R_4 = (-1.0/32.0)*R_4; R_9 = R_4+R_9;\n";
                os << "    R_11 = R_6*R_11; R_11 = (-1.0/160.0)*R_11; R_9 = R_11+R_9; r_x += R_9;\n";
                break;
            default:
                os << "TODO;\n";
            }
            ...
            os << "TODO: several coeffs with coeffs.cuts.size();\n";
        }

        os << "for( size_t i1 = 0, i0 = _cuts.size() - 1; i1 < _cuts.size(); i0 = i1++ ) {\n";
        os << "    if ( _cuts[ i0 ].seg_type == SegType::arc )\n";
        os << "        arc_val_centroid( ctd.x, ctd.y, _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );\n";
        os << "    else\n";
        os << "        seg_val_centroid( ctd.x, ctd.y, _cuts[ i0 ].point - _sphere_center, _cuts[ i1 ].point - _sphere_center );\n";
        os << "}\n";
    } else {
        os << "    static const std::vector<std::pair<TF,std::array<TF," << coeffs.cuts[ 0 ].coeffs.size() << ">>> coeffs_centroid = {\n";
        for( std::size_t num_cut = 0; num_cut < coeffs.cuts.size(); ++num_cut ) {
            os << "        { " << std::setprecision( 10 ) << std::setw( 16 ) << coeffs.cuts[ num_cut ].position << ", { ";
            for( std::size_t i = 0; i < coeffs.cuts[ num_cut ].coeffs.size(); ++i )
                os << ( i ? ", " : "" ) << std::setprecision( 10 ) << std::setw( 16 ) << coeffs.cuts[ num_cut ].coeffs[ i ];
        ...
        os << "    };\n";
        os << "    \n";
        os << "    _r_centroid_integration( ctd.x, ctd.y, coeffs_centroid );\n";
    }
}

template<class Func>
void centroid_integration( const Func &func, double precision, const std::vector<TF> &r, int degp ) {
    // evaluation of the function
    std::vector<TF> r2( r.size() );
    std::vector<TF> y( r.size() );
    for( size_t i = 0; i < r.size(); ++i ) {
        r2[ i ] = r[ i ] * r[ i ];
        y[ i ] = func( r[ i ] );
    ...
    // get a piecewise polynomial approximation of y( r2 )
    UnivariatePiecewisePolynomialApproximator<TF> ap_func;
    ap_func.precision = precision / 10;
    ap_func.degp = 7; // we're using high precision number, so that it's legitimate to use high degree polynomials
    auto pa_f = ap_func.run( r2, y );

    // get points of Py (primitive of y)
    TF acc = 0;
    std::vector<TF> py;
    for( std::size_t num_point = 0, num_cut = 0; num_point < r.size(); ++num_point ) {
        while ( r2[ num_point ] >= pa_f.cuts[ num_cut ].position ) {
            TF beg = num_cut ? pa_f.cuts[ num_cut - 1 ].position : 0;
            TF end = pa_f.cuts[ num_cut ].position;
            for( std::size_t i = 0; i < pa_f.cuts[ num_cut ].coeffs.size(); ++i )
                acc += pa_f.cuts[ num_cut ].coeffs[ i ] / ( i + 1 ) * ( pow( end, i + 1 ) - pow( beg, i + 1 ) );
            ++num_cut;
        }
        ...
        TF beg = num_cut ? pa_f.cuts[ num_cut - 1 ].position : 0;
        for( std::size_t i = 0; i < pa_f.cuts[ num_cut ].coeffs.size(); ++i )
            v += pa_f.cuts[ num_cut ].coeffs[ i ] / ( i + 1 ) * ( pow( r2[ num_point ], i + 1 ) - pow( beg, i + 1 ) );
        py.push_back( v );
    }
    ...
    //        return res;
    //    };
    //    matplotlibcpp::plot( to_double( r2 ), to_double( py ) );
    //    matplotlibcpp::show();
    ...
    auto pa_i = ap_is.run( r2, py );

    // code gen
    gen_int_centroid( std::cout, pa_i );
}

template<class Func>
int integration( const Func &func, double precision, const std::vector<TF> &r, int degp, bool centroid ) {
    if ( centroid ) {
        div_inv_integration( func, precision, r, degp, centroid );
        centroid_integration( func, precision, r, degp );
    } else {
        div_inv_integration( func, precision, r, degp, centroid );
    }
    return 0;
...
int run( std::string function_name, double precision, double beg_log_scale, double end_log_scale, int nb_points_reg_scale, int nb_points_log_scale, int degp, bool centroid ) {
    // r points
    std::vector<TF> r( nb_points_reg_scale + nb_points_log_scale );
    for( size_t i = 0; i < nb_points_reg_scale; ++i )
        r[ i ] = ( i + 0.5 ) * beg_log_scale / nb_points_reg_scale;
    for( size_t i = 0; i < nb_points_log_scale; ++i )
        r[ nb_points_reg_scale + i ] = beg_log_scale * pow( end_log_scale / beg_log_scale, TF( i + 0.5 ) / ( nb_points_log_scale - 1 ) );

    // generation
    if ( function_name == "GaussianVariance" ) return integration( []( auto r ) { return r * r * exp( - r * r ); }, precision, r, degp, centroid );
    if ( function_name == "Gaussian"         ) return integration( []( auto r ) { return exp( - r * r );         }, precision, r, degp, centroid );
    if ( function_name == "Unit"             ) return integration( []( auto r ) { return 1;                      }, precision, r, degp, centroid );
    if ( function_name == "R2"               ) return integration( []( auto r ) { return r * r;                  }, precision, r, degp, centroid );
    std::cerr << function_name << " is not a known function type";
    return 1;
}

int main( int argc, char **argv ) {
    cxxopts::Options options( argv[ 0 ], "Makes a piecewise polynomial of a function which div is the one given in argument + optional version to get centroid");
    options.add_options()
        ( "r,nb-points-in-reg-scale", "number of point in the regular scale (between 0 and --beg-l)", cxxopts::value<int>()->default_value( "100" ) )
        ( "l,nb-points-in-log-scale", "number of point in the log scale (between --beg-l and --end-l)", cxxopts::value<int>()->default_value( "100" ) )
        ( "d,polynomial-degree"     , "degree of the polynomials", cxxopts::value<int>()->default_value( "7" ) )
        ( "b,beg-log-scale"         , "begining of the points in the log scale (= end of the points in the regular scale)", cxxopts::value<double>()->default_value( "4" ) )
        ( "e,end-log-scale"         , "end of the points in the log scale", cxxopts::value<double>()->default_value( "1e10" ) )
        ( "p,precision"             , "target precision (norm inf)", cxxopts::value<double>()->default_value( "1e-10" ) )
        ( "f,function"              , "function name (e.g. Gaussian, ...)", cxxopts::value<std::string>() )
        ( "c,centroid"              , "generate functions to compute the centroid" )
        ( "h,help"                  , "get help" )
        ;
    ...
    return run(
        args[ "function"               ].as<std::string>(),
        args[ "precision"              ].as<double     >(),
        args[ "beg-log-scale"          ].as<double     >(),
        args[ "end-log-scale"          ].as<double     >(),
        args[ "nb-points-in-reg-scale" ].as<int        >(),
        args[ "nb-points-in-log-scale" ].as<int        >(),
        args[ "polynomial-degree"      ].as<int        >(),
        args.count( "centroid" )
    );
}
